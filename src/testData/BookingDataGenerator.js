...
export class BookingDataGenerator {
  ...
  static getRequestBody(options = {}) {
    ...
    const totalprice = options.totalprice || faker.number.int({ min: 100, max: 1000 });
    const depositpaid = options.depositpaid !== undefined ? options.depositpaid : true;
    const checkin = options.checkin || DateTime.now().toFormat('yyyy-MM-dd');
    const daysToStay = options.daysToStay || 5;
    const checkout = options.checkout || DateTime.fromFormat(checkin, 'yyyy-MM-dd').plus({ days: daysToStay }).toFormat('yyyy-MM-dd');
    const additionalneeds = options.additionalneeds || faker.helpers.arrayElement(['Breakfast', 'Lunch', 'Dinner', 'Extra pillows']);
